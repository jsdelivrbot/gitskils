<template>
  ...
  <div>
    ...
    <app-header title="所提问题列表"></app-header>
    <div class="zjdf-main">
        ...
        <ul class="zjdf-uls">
          <li class="zjdf-li" v-for="list in lists">
            ...
            <div class="question-text">
              <div class="text-contents">{{list.contTxt}}</div>
              <div class="float-r">
                <mt-button type="primary" size="small" class="look" @click="lookfn(list.questCode)">查看</mt-button>
                <mt-button type="primary" size="small" class="answer" @click="remove(list.questCode)">删除</mt-button>
              </div>
            </div>
          </li>
        </ul>
  ...
  export default {
    ...
    methods: {
      ...
      initList(RowNum,updown){
        let self = this;
        httpGetMethod("hrs-ess/app/getMyQuestionList.action?",{
          RowNum:RowNum,//页码
      ...
      lookfn:function(questCode){
        window.location.href = "#/wyzxdetail/dafu/" + questCode;
      },
      remove: function (questCode) {
        let self = this;
        httpGetMethod("hrs-ess/app/deleteAskQuestion.action?", {
          questCode: questCode,
        }, function(result) {
          if(result.success) {
            Tool.alert("删除成功",'','',function () {
              self.lists=[];
              self.initList(self.RowNum,"1");
            });
