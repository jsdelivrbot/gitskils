<template>
	...
	<div>
		...
		<div class="xcxx-main" v-if="flag==3">
			...
			<!--<input style="width: 100%;height: 40px;border: 1px solid #d8d8d8;" type="text" v-model="token" id="" value="" />-->

			<div class="top">
				<div class="top-content">
					<div class="top-m">
						<div class="top-m-t">应发金额</div>
						<div class="top-m-m"><span v-if='isycje'>{{wageTotal}}</span><span v-if="!isycje">****</span> &nbsp<span class="top-m-zt">元</span></div>
...
<script>
  ...
  import { modelStatisticalAnalysis } from "../../utils/base.js"; //模块统计分析
	...
	export default {
			...
			getInfo: function() {
				...
				httpGetMethod("hrs-ess/app/seachWageInfo.action?", {
					...
					if(result.success) {
						...
						self.wageTotal = _result['wageTotal'];
						self.menuKeys.forEach(function(item,index) {

							if(item.titleId=='jjWageList'){
								var jjWageListdata = _result[item.titleId];

								jjWageListdata.forEach(function(opt,index){

									var objs = {
										title: Tool.formate(new Date(self.currentmonth),"yyyy年MM月") + opt['day'] + "日  " + item.title,
										titleId: item.titleId,
										data:opt['data'],
										total:"",
										isshow: false
									}
									self.currentdata.push(objs);
								})
							}else{
								if(_result[item.titleId]['size']){
									var obj = {
										title:item.title,
										titleId: item.titleId,
										data:_result[item.titleId]['data'],
										total:_result[item.titleId]['total'],
										isshow: false
									}
									self.currentdata.push(obj);
								}
		...
		mounted: function() {
      if (window.sessionStorage.getItem("XinChouStatisticalAnalysis") == null || window.sessionStorage.getItem("XinChouStatisticalAnalysis") == false) {
        //薪酬信息模块统计接口
        modelStatisticalAnalysis("003", "薪酬信息", "XinChouStatisticalAnalysis");
      }
			/**********************************************
			       逻辑流程：
			       1、验证用户是否设置过密码
