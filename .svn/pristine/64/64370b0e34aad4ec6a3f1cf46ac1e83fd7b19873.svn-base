<template>
	...
	<div>
      ...
      <mt-popup v-model="sheetVisible" position="bottom" class="xitong-wrapper">
        ...
        <div>
          <mt-button type="default" @click="cancelTxl">取消</mt-button>
          <mt-button type="primary" @click="enSureTxl">确定</mt-button>
        </div>
...
//          this.deptCodes(_index0,_index1);
      ...
      enSureTxl : function () {
        this.enterprise = this.values[1];
        let _index0 = this.slots[0].values.indexOf(this.values[0]);
        let _index1 = this.slots[1].values.indexOf(this.values[1]);
        _index1 = (_index1 == -1) ? 0 : _index1;
        this.deptCodes(_index0,_index1);
