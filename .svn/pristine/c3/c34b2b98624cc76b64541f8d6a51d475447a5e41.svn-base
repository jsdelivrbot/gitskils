<template>
  ...
  <div class="Xcs-div-main">
    ...
    <div class="XCSsearch" v-if="!noresult">
    ...
    <!--列表-->
    <div class="Xcslist-main" :style="pageh" v-if="!noresult">
      <mt-loadmore :auto-fill="false" :top-method="loadTop" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" ref="loadmore" :top-distance='10' bottom-pull-text="上拉加载更多" bottom-drop-text="释放加载" :style="pagesh">
        ...
        <div style="height: 32px;line-height: 32px;" v-if='XcsList.length>10 && allLoaded'>已加载到底部</div>
      </mt-loadmore>
    </div>
      ...
      loadTop: function() {
        ...
        this.getdata(function(lt) {
          ...
          lt.forEach(function(item) {
            item.path = "/ylbxdetail/20/" + item.noticeCode;
...
//        this.getdata();
    ...
    mounted: function() {
      if (window.sessionStorage.getItem("XiaoChangShiStatisticalAnalysis") == null || window.sessionStorage.getItem("XiaoChangShiStatisticalAnalysis") == false) {
        //小常识模块统计接口
        modelStatisticalAnalysis("016", "小常识", "XiaoChangShiStatisticalAnalysis");
      }
      let self = this;
      this.getSxData();
      this.getdata(function(lt){
