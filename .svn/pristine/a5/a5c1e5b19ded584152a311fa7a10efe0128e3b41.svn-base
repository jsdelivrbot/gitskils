<template>
	...
	<div>
      ...
      <div class="jyfxlist-main" :style="pagesearch">
        <mt-loadmore v-if="!noresult" :top-method="loadTop" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" ref="loadmore" :auto-fill="false" :top-distance='10' bottom-pull-text="上拉加载更多" bottom-drop-text="释放加载" :style="pagesShow">
        		<ul class="jyfx-list">
...
<script>
  ...
  export default {
    ...
    mounted(){
      if (window.sessionStorage.getItem("JingYanFenXiangStatisticalAnalysis") == null || window.sessionStorage.getItem("JingYanFenXiangStatisticalAnalysis") == false) {
        //经验分享模块统计接口
        modelStatisticalAnalysis("034", "经验分享", "JingYanFenXiangStatisticalAnalysis");
      };
      //列表初始化
      let self = this;
    ...
    methods: {
      ...
      jyfxdetail(noticeCode){
        window.location.href = "#/ylbxdetail/30/" + noticeCode;
      },
      //获取点赞、评论条数
      noticeload(){
        let self = this;
