<template>
	<div>
		<app-header title="我要提问" ></app-header>
		...
		<div class="wyzx-main" :style="pageh">
			...
			<!--<mt-picker :slots="slots" @change="onValuesChange" v-show="sheetVisible" @click="choiceType(picker)">-->
        <!---->
      <!--</mt-picker>-->
      <mt-popup v-model="sheetVisible" position="bottom" class="xitong-wrapper">
        <div>
          <mt-picker :slots="slots" @change="onValuesChange"></mt-picker>
        </div>
        <div>
          <mt-button type="default" @click="cancelQuestion">取消</mt-button>
          <mt-button type="primary" @click="enSureQuestion">确定</mt-button>
        </div>
  ...
  import { modelStatisticalAnalysis } from "../../../utils/base.js"; //模块统计分析
	...
	export default {
      ...
      AppHeader
		...
		mounted(){
      if (window.sessionStorage.getItem("WoYaoTiWenStatisticalAnalysis") == null || window.sessionStorage.getItem("WoYaoTiWenStatisticalAnalysis") == false) {
        //我要提问模块统计接口
        modelStatisticalAnalysis("015", "我要提问", "WoYaoTiWenStatisticalAnalysis");
      }
			let self = this;
      ...
      enSureQuestion : function () {
			  let self = this;
        self.qstype = self.qstypes[0];
        self.lists.forEach(function(item){
          if(self.qstype == item.NAME)
          {
            self.qscode = item.CODE;
          }
        });
